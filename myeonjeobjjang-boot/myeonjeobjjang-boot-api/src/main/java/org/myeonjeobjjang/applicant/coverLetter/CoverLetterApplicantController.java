...
public class CoverLetterApplicantController {
    ...
    @PostMapping("/empty")
    public ResponseEntity<IntegrationCoverLetterResponse.IntegrationCoverLetterInfoResponse> createEmpty(@AuthenticationPrincipal PrincipalDetails principalDetails) {
        Member member = principalDetails.getMember();
        return ResponseEntity.ok(coverLetterService.createEmpty(member));
    }

    @PostMapping("/jobDescriptions/{jobDescriptionId}")
    public ResponseEntity<IntegrationCoverLetterResponse.IntegrationCoverLetterInfoResponse> createCopy(@AuthenticationPrincipal PrincipalDetails principalDetails, @PathVariable Long jobDescriptionId) {
        Member member = principalDetails.getMember();
        return ResponseEntity.ok(coverLetterService.createWithJobDescription(member, jobDescriptionId));
    }

    @GetMapping("/{coverLetterId}")
    public ResponseEntity<IntegrationCoverLetterResponse.IntegrationCoverLetterInfoResponse> get(@PathVariable Long coverLetterId) {
